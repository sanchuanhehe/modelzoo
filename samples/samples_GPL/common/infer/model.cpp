...
const std::unordered_map<ModelType, std::pair<ProcessFunc, ProcessFunc>> Model::modelTypeToProcessMap_ = {
    { ModelType::Resnet50, {std::bind(ImageProcess, std::placeholders::_1, std::placeholders::_2,
                    std::placeholders::_3, ImageprocessOptions(256, 224, true)), PrintTop5}},
    { ModelType::Yolov3, {Yolo3Preprocess, GetYolo3Box}},
    { ModelType::Yolov7, {std::bind(Yolov7Preprocess, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, true),
        std::bind(GetYoloV7Box, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, true)}},
    ...
    { ModelType::Unet, {std::bind(UnetPreprocess, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3,
                            ImageprocessOptions(572, -1, true)), UnetPostprocess} }
};
#else
const std::unordered_map<ModelType, std::pair<ProcessFunc, ProcessFunc>> Model::modelTypeToProcessMap_ = {
    { ModelType::Resnet50, {std::bind(ImageProcess, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3,
        ImageprocessOptions(256, 256, false)), PrintTop5}},
    { ModelType::VGG16, {std::bind(ImageProcess, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3,
        ImageprocessOptions(256, 224, false)), PrintTop5}},
    { ModelType::Yolov7, {std::bind(Yolov7Preprocess, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, false),
          std::bind(GetYoloV7Box, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, false)}},
    { ModelType::Yolov6s, {Yolo6s::Yolov6sPreprocess, Yolo6s::Yolov6sPostprocess} },
    { ModelType::Yolov11sPose, {Yolo11sPoseNS::Yolov11sPosePreprocess, Yolo11sPoseNS::Yolov11sPosePostprocess} }
};
#endif
...
std::vector<std::vector<Tensor>> Model::Infer(const std::string& filePath,
  ...
  for (size_t i = 0; i < param.fileLists.size(); ++i) {
    if (preprocessFunc_ == nullptr ||
        !preprocessFunc_(param.fileLists[i], inBuf, mdlInputDescs_)) {
    ...
    if (postprocessFunc_ == nullptr ||
        !postprocessFunc_(param.fileLists[i], outBuf, mdlOutputDescs_)) {
    ...
    LOG(INFO) << std::fixed << std::setprecision(2) << "already infer " << i + 1
              ...
              << "% remains, "
              << costTime * (param.fileLists.size() - i - 1) * 1.00f /
                     (i * S2MIN + S2MIN)
              << "min remains.";
  }
  float msDur = static_cast<float>(dur.count()) / (param.loop * param.fileLists.size() * MS2S); // 	1 millisecond = 1000 microseconds
  LOG(INFO) << std::fixed << std::setprecision(2) << "execution time: " << msDur
            << "ms, frame rate: " << (MS2S / msDur) << "fps";
  return outputs;
}
