...
static int originHeight = 384;

constexpr int YOLO_INPUT_HEIGHT = 384;
constexpr int YOLO_INPUT_WIDTH = 640;
constexpr int NUM_CLASSES = 80;
constexpr int BBOX_COORD_NUM = 4; // bbox包含坐标的个数（x, y, w, h）
constexpr int BBOX_SIZE = BBOX_COORD_NUM + NUM_CLASSES; // bbox：xywh 模型预测类型的score
constexpr int BBOX_NUM = (YOLO_INPUT_HEIGHT / 8) * (YOLO_INPUT_WIDTH / 8) +
    (YOLO_INPUT_HEIGHT / 16) * (YOLO_INPUT_WIDTH / 16) + (YOLO_INPUT_HEIGHT / 32) * (YOLO_INPUT_WIDTH / 32);
...
// YOLO 输出格式相关常量
constexpr int BOX_CX_INDEX = 0;
constexpr int BOX_CY_INDEX = 1;
constexpr int BOX_W_INDEX = 2;
constexpr int BOX_H_INDEX = 3;
...
{
    BBox boxNew;
    float scale = std::min((float)YOLO_INPUT_HEIGHT / originHeight,
        (float)YOLO_INPUT_WIDTH / originWidth);
    ...
    boxNew.x1 = std::max(0.0f, std::min((box.x1 - dx) / scale, (float)originWidth));
    boxNew.y1 = std::max(0.0f, std::min((box.y1 - dy) / scale, (float)originHeight));
    boxNew.x2 = std::max(0.0f, std::min((box.x2 - dx) / scale, (float)originWidth));
    boxNew.y2 = std::max(0.0f, std::min((box.y2 - dy) / scale, (float)originHeight));
...
static void Transpose(float* data, int rows, int cols)
{
    ...
    memcpy(data, temp.data(), totalElements * sizeof(float));
}
...
static std::vector<BBox> GetBBox(std::vector<TensorBuf>& outBufs)
{
    ...
    for (int i = 0; i < BBOX_NUM; i++) {
        // 提取类别分数
        const float* scores = data + BBOX_COORD_NUM; /* 获取预测类别的概率入口点 */
        int classId = std::max_element(scores, scores + NUM_CLASSES) - scores;
        float conf = scores[classId];

        // 获取x y x y
        float x1 = data[BOX_CX_INDEX] - data[BOX_W_INDEX] / HALF_DIVISOR;
        float y1 = data[BOX_CY_INDEX] - data[BOX_H_INDEX] / HALF_DIVISOR;
        float x2 = data[BOX_CX_INDEX] + data[BOX_W_INDEX] / HALF_DIVISOR;
        float y2 = data[BOX_CY_INDEX] + data[BOX_H_INDEX] / HALF_DIVISOR;
...
bool GetFireDetectionBox(std::vector<std::string>& fileList,
