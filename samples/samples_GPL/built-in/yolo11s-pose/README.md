...
YOLO系列网络模型是最为经典的one-stage算法，也是目前工业领域使用最多的目标检测网络，YOLO11网络模型是YOLO系列的最新版本，在继承了原有YOLO网络模型优点的基础上，在架构和训练方法上进行了重大改进，具有更高的检测精度、速度和效率。yolo11s-pose作为yolo11的姿态估计的模型，能检测出代表人体不同部位的17个关键点。
本示例使用yolo11s

- 参考实现：

  ```
  https://github.com/ultralytics/ultralytics/blob/main/ultralytics/cfg/models/11/yolo11-pose.yaml
  ```
...
├── script
│   ├── yolo11s_pose_preprocess.py     //数据预处理脚本
│   ├── yolo11s_pose_infer.py          //模型推理脚本
│   ├── yolo11s_pose_postprocess.py    //数据后处理脚本
│   ├── yolo11s_pose_evaluate.py       //精度评估脚本
│   ├── yolo11s_pose_pth2onnx.py       //pytorch转onnx脚本
...
2. 导出onnx文件。
      ...
      # 在yolo11s-pose代码根目录下执行
      cd script 
      python3 yolo11s_pose_pth2onnx.py # 执行成功后，onnx模型会生成到../model/yolo11s-pose.onnx
      cd ..
      ```
      在安装ultralytics后，也可以使用下述命令直接导出onnx
      ```bash
      yolo export model="yolo11s-pose.pt" format=onnx opset=13 project=export name="yolo11s-pose.onnx"
      ```
...
3. 使用ATC工具将ONNX模型转OM模型。
      ...
      1. SS928V100 SVP_NNN上的om模型转换命令
         ...
         atc ----framework=5 --model="./model/yolo11s-pose.onnx" --input_shape="images:1,3,640,640" --output="model/yolo11s-pose" --image_list="data/preprocess/bin/000000006818.bin" --soc_version=SS928V100 --compile_mode=6
         ```
...
1. 精度验证。
   ...
   ```bash
   python accuracy.py --bin_dir ../coco/result/bin --img_dir ../coco/val2017 --output_json ../coco/result.json --gt_annotations ../coco/annotations/person_keypoints_val2017.json
   cd script # 切换到script目录
   python3 yolo11s_pose_evaluate.py \
      --result_dir "../out/result/txt" \
      --gt_annotations "../../../../../datasets/coco2017/annotations/person_keypoints_val2017.json"
   ```
   ...
   Average Recall     (AR) @[ IoU=0.50:0.95 | area=medium | maxDets= 20 ] = 0.584
   Average Recall     (AR) @[ IoU=0.50:0.95 | area= large | maxDets= 20 ] = 0.748
   ```
...
2. 验证batch_size的om模型的性能，参考命令如下：
   ...
   ```
   execution time: 25.09ms, frame rate: 39.85fps
   ```
   在板端会输出显示，NNN平台上性能结果如下：
   ```
   execution time: 45.41ms, frame rate: 22.02fps
   ```
...
1. 使用python脚本进行数据预处理，将原始数据集转换为模型的输入数据。
   ...
   ```bash
   cd script # 切换到script目录
   python3 yolo11s_pose_preprocess.py \
...
2. 使用python脚本在PC上进行模型推理（可以跟在开发板上的推理结果进行对比）
   ```bash
   cd script # 切换到script目录
   python3 yolo11s_pose_infer.py \
...
4. 使用python脚本进行模型推理结果的后处理（解码bin文件、置信度过滤、分类NMS等操作）
   ```bash
   cd script # 切换到script目录
   python3 yolo11s_pose_postprocess.py \
...
调用ACL接口推理计算，yolo11s-pose模型的性能和精度参考下列数据。

| 芯片型号    | Batch Size | 数据集   | mAP（keypoints IoU=0.50:0.95） | mAP（keypoints IoU=0.50） | 性能（fps） |
| ----------- | ---------- | -------- | ------------------ | ------------------ | ------------------ |
| Hi3403V100 SVP_NNN | 1          | coco2017  | 56.6%       | 84.2%         | 39.85 |
| Hi3403V100 NNN | 1          | coco2017  | 57.0%       | 84.2%         | 22.02 |
