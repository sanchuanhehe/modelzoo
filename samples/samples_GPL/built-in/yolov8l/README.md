...
4. 切换到可执行文件main所在的目录，运行可执行文件。
    ...
    ```
    ./main --acl ../src/acl.json --model ../model/yolov8l.om --input ../data/file_list_1.json
    ```
...
2. 获取权重文件。
   ...
   在[链接](https://github.com/ultralytics/assets/releases/tag/v0.0.0)中找到yolov8l.pt下载，存储至 model。
      ```
      mkdir -p model
      ```
...
1. 切换到可执行文件main所在的目录，数据集上运行命令参考：
   ...
   ```
   ./main --model ../model/yolov8l.om --input ../data/file_list.json
   ```
...
3. 验证om模型的性能，参考命令如下：
   ```
   ./main --model ../model/yolov8l.om --input ../data/file_list_1.json
   ```
...
调用ACL接口推理计算，yolov8l模型的性能和精度参考下列数据。

| 芯片型号    | Batch Size | 数据集   | AP（IoU=0.50） | AP（IoU=0.50:0.95） | 性能（fps） |
| ----------- | ---------- | -------- | ------------------ | ------------------ | ------------------ |
| Hi3403V100 SVP_NNN | 1          | coco2017  | 69.0%       | 51.5%         | 10.656 |
