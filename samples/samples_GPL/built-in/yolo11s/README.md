...
1. 获取原始数据集。（解压命令参考tar –xvf *.tar与 unzip *.zip）
   ...
   该模型使用 [coco2017 val数据集](https://cocodataset.org/#download) 进行精度评估，在`yolo11s`源码根目录下新建`coco`文件夹，数据集放到`coco`里，文件结构如下：
...
2. 导出onnx文件。
      ...
      ```
      yolo export model="yolo11s.pt" format=onnx opset=13 project=export name="yolo11s.onnx"
      ```
...
3. 验证batch_size的om模型的性能，参考命令如下：
   ...
   ```
   Hi3403V100 NNN上的执行命令,file_list_1.json中loop设为100
   ```
   ./main --acl ../src/acl.json --model ../model/yolo11s.om --input ../data/file_list_1.json
   ```
...
| ----------- | ---------- | -------- | ------------------ | ------------------ | ------------------ |
| Hi3403V100 SVP_NNN | 1          | coco2017  | 62.8%       | 45.3%         | 42.607 |
| Hi3403V100 NNN | 1          | coco2017  | 62.4%       | 43.6%         | 23.02 |
