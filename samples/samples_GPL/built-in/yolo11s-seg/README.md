...
YOLO11s-seg 是一款轻量级 one-stage 实例分割模型，沿用 C2f 骨干网络与改进型 PAFPN 颈部结构，采用 “原型掩码 + 动态系数” 的分割头设计，在保持快速推理性能的同时，可精准实现像素级实例分割，适合轻量化轻量化边缘计算场景。
...
├── script
│   ├── yolo11s_seg_pth2onnx.py       //pytorch模型转onnx模型脚本
│   ├── yolo11s_seg_preprocess.py     //数据预处理脚本
│   ├── yolo11s_seg_infer.py          //模型推理脚本
│   ├── yolo11s_seg_postprocess.py    //数据后处理脚本
│   ├── yolo11s_seg_evaluate.py       //精度评估脚本
...
├── model
│   ├── yolo11s-seg.onnx	      //onnx模型文件
│   ├── yolo11s-seg.om	         //om模型文件
...
2. 导出onnx文件。
      ...
      使用./script/yolo11s_seg_pth2onnx.py导出onnx模型到../model/yolo11s-seg.onnx

      ```bash
      cd script
      python3 yolo11s_seg_pth2onnx.py
      cd ..
      ```
...
3. 使用ATC工具将ONNX模型转OM模型。
      ...
      1. SS928V100 SVP_NNN上的om模型转换命令
         ...
         atc --framework=5 --model="model/yolo11s-seg.onnx" --input_shape="images:1,3,640,640" --output="model/yolo11s-seg" --soc_version=SS928V100 --image_list="data/preprocess/bin/000000006818.bin" --compile_mode=6
         ```
      2. SS928V100 NNN上的om模型转换命令
         ```bash
         atc --framework=5 --model="model/yolo11s-seg.onnx" --input_shape="images:1,3,640,640" --output="model/yolo11s-seg" --enable_single_stream=true --input_fp16_nodes="images" --soc_version=OPTG
         ```
         
         运行成功后生成yolo11s-seg.om模型文件。
...
1. 精度验证。
   ...
   python3 yolo11s_seg_evaluate.py \
...
2. 验证batch_size的om模型的性能，参考命令如下：
   ...
   ```bash
   execution time: 29.36ms, frame rate: 34.06fps
   ```
   在板端会输出显示，NNN平台上性能结果如下：
   ```bash
   execution time: 55.00ms, frame rate: 18.18fps
   ```
...
1. 使用python脚本进行数据预处理，将原始数据集转换为模型的输入数据。
   ...
   ```bash
   cd script # 切换到script目录
   python3 yolo11s_seg_preprocess.py \
...
4. 使用python脚本进行模型推理结果的后处理（解码bin文件、置信度过滤、分类NMS等操作）
   ```bash
   cd script # 切换到script目录
   python3 yolo11s_seg_postprocess.py \
...
| ----------- | ---------- | -------- | ------------------ | ------------------ | ------------------ |
| Hi3403V100 SVP_NNN | 1          | coco2017  | 45.1%       | 36.6%         | 34.06 |
| Hi3403V100 NNN | 1          | coco2017  | 45.7%       | 36.4%         | 18.18 |
