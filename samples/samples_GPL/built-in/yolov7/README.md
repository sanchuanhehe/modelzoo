...
│   ├── generate_quant_bin.py     //python执行脚本
...
1. 获取原始数据集。（解压命令参考tar –xvf *.tar与 unzip *.zip）
   ...
   该模型使用 [coco2017 val数据集](https://cocodataset.org/#download) 进行精度评估，在`yolov7`源码根目录下新建`datasets`文件夹，数据集放到`datasets`里，文件结构如下：
...
3. 导出onnx文件。
    ...
    1. 使用开源源码中的导出方法
         ...
         ```
         cd yolov7
         python3 export.py --weights ../model/yolov7.pt --grid --simplify
         cd ../
         ```
...
3. 验证om模型的性能，参考命令如下：
    ...
    参数说明：(此模式下，file_list_1.json只放一张图片, 文件中 loop设为1000)
    ...
    NNN平台上性能结果如下：
    ```
     execution time: 130.77ms, frame rate: 7.65fps
    ```
