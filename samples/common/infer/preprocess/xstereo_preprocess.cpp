...
namespace Infer
{
    static Result ReadImgFileToBuf(cv::Mat &chwImg, TensorDesc &desc, TensorBuf &inBuf)
    {
        ...
        //  ReadImgFileToBuf: loopTimes 1152 linesize: 1248 sizeof(float): 4
        ...
        // 查看 cv::Mat 的详细信息
LOG(INFO) << "cv::Mat info:";
LOG(INFO) << "  rows: " << chwImg.rows;
LOG(INFO) << "  cols: " << chwImg.cols;
LOG(INFO) << "  channels: " << chwImg.channels();
LOG(INFO) << "  depth: " << chwImg.depth();  // CV_8U=0, CV_8S=1, CV_16U=2, ...
LOG(INFO) << "  type: " << chwImg.type();    // CV_8UC3=16, CV_32FC1=5, ...
LOG(INFO) << "  dims: " << chwImg.dims;      // 维度数

// 计算总字节数
size_t totalBytes = chwImg.total() * chwImg.elemSize();
LOG(INFO) << "  total elements: " << chwImg.total();
LOG(INFO) << "  element size: " << chwImg.elemSize() << " bytes";
LOG(INFO) << "  total bytes: " << totalBytes << " bytes";

        LOG(INFO) << "ReadImgFileToBuf: loopTimes " << loopTimes << " linesize: " << lineSize << " sizeof(float): " << dataSize << " dataSize: " << inBuf.size;
        
        // Loop 730: destPtr=0x7fb321e6c0, srcPtr=0x7fb2956700, width=1248, strideElemNum=1248, totalSize=1248
        ...
        for (int64_t loop = 0; loop < loopTimes; loop++) {
            ...
            if (static_cast<char *>(inBuf.GetRawPtr()) + loop * strideElemNum > static_cast<char *>(inBuf.GetRawPtr()) + inBuf.size) {
...
static cv::Mat loadImage(const std::string &imgPath, int target_w = 1248, int target_h = 384, bool rect_mode = true) {
    ...
    if (pasteRoi.width <= 0 || pasteRoi.height <= 0) {
        throw std::runtime_error("Invalid paste ROI: " + std::to_string(pasteRoi.width) + "," + std::to_string(pasteRoi.height));
    }
...
bool XStereoPreprocess(std::vector<std::string>& fileList, std::vector<TensorBuf>& inBufs, std::vector<TensorDesc>& inDescs)
    {
        ...
        std::vector<std::string> imgPaths= GetInputList(fileList[0]);
