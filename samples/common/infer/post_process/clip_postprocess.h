...
bool ClipTxtPostprocess(std::vector<std::string> &fileList,
                        std::vector<Infer::TensorBuf> &tensorBufs,
                        std::vector<Infer::TensorDesc> &tensorDescs);
...
StackDim(std::vector<std::vector<float>> &vectors);

std::vector<float>
MatmulWithScale(const std::vector<float> &image_feats,
