...
cv::Mat Postprocess(const float *preds, int originalHeight, int originalWidth) {
  ...
  for (int c = 0; c < CHANNEL_NUM; ++c) {
    channels[c] =
        cv::Mat(TARGET_HEIGHT, TARGET_WIDTH, CV_32F, chwMat.ptr<float>(c))
            .clone();
  }
  cv::Mat hwcMat;
  ...
  if (originalHeight != TARGET_HEIGHT || originalWidth != TARGET_WIDTH) {
    cv::resize(uint8Mat, uint8Mat, cv::Size(originalWidth, originalHeight), 0,
               0, cv::INTER_LINEAR);
  }
...
std::vector<float> MeanByCol(std::vector<std::vector<float>> &feats) {
  ...
  for (int d = 0; d < FEATDIM; ++d) {
    ...
    for (const auto &row : feats) {
      if (row.size() != FEATDIM) {
        throw std::invalid_argument("All rows must have length " +
                                    std::to_string(FEATDIM));
      }
