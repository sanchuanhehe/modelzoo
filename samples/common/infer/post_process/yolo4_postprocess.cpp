    ...
    for (auto& pair : classBoxes) {
        ...
        for (size_t i = 0; i < clsBoxes.size(); ++i) {
            ...
            for (size_t j = i + 1; j < clsBoxes.size(); ++j) {
                ...
                if (iou > iouThres) keep[j] = false;
            }
        }

        // 保留有效框
        for (size_t i = 0; i < clsBoxes.size(); ++i) {
            if (keep[i]) nmsResult.push_back(clsBoxes[i]);
        }
    }
...
void SortResults(vector<BBox>& results) {
    sort(results.begin(), results.end(), [](const BBox& a, const BBox& b) {
        if (a.cocoClassId != b.cocoClassId) return a.cocoClassId < b.cocoClassId;
        return a.score > b.score;
    });
}
...
// -------------------------- 内部辅助函数：直接解码原始NCHW数据 --------------------------
static bool DecodeRawOutputLayer(TensorBuf& buf, int layerIdx, float confThres,
    ...
    float* rawData = reinterpret_cast<float*>(buf.GetRawPtr());
    if (rawData == nullptr) {
    ...
    for (int anchorIdx = 0; anchorIdx < SCALE_SIZE; ++anchorIdx) {
        ...
        for (int y = 0; y < gridSize; ++y) {  // 遍历网格高度（y轴）
            for (int x = 0; x < gridSize; ++x) {  // 遍历网格宽度（x轴，仅取前gridSize列，跳过冗余填充）
                ...
                if (finalConf < confThres) continue;
...
bool Yolov4Postprocess(vector<string>& fileList,
    ...
    for (size_t imgIdx = 0; imgIdx < fileList.size(); ++imgIdx) {
        ...
        LOG(INFO) << "\n====== 处理图像：" << imgPath << "（" << imgIdx + 1 << "/" << fileList.size() << "）======";

        // 保存原始推理结果（可选）
        if (!saveBin.empty()) {
            for (size_t i = 0; i < outBufs.size(); ++i) {
                string binPath = saveBin + "/" + fileName + "_output_" + to_string(i) + ".bin";
                ofstream(binPath, ios::binary).write(
                    reinterpret_cast<char*>(outBufs[i].GetRawPtr()), outBufs[i].size
                );
            }
        ...
        bool valid = true;
        for (int i = 0; i < 3; ++i) {
            if (!DecodeRawOutputLayer(outBufs[i], i, confThres, imgWidth, imgHeight, boxes)) {
                valid = false;
                break;
            }
        }
