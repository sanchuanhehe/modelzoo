...
CRNN是卷积循环网络，本模型是一个基于其的中文 OCR 模型。
- 参考实现：
...
- 输入数据
  ...
  | 输入数据 | 数据类型 | 大小             | 数据排布格式 |
  | -------- | -------- | ---------------- | ------------ |
  | image    | RGB_FP32 | 1 x 3 x 32 x 160 | NCHW         |

- 输出数据

  | 输出数据 | 数据类型 | 大小   |
  | -------- | -------- | ------ |
  | class    | FP32     | 41x1x6736 |
...
├── script
│   ├── pth2onnx.py     //pth转换onnx脚本
│   ├── accuracy.py     //精度计算脚本
│   ├── generate_quant_bin.py     //atc量化文件工具
...
网站上提供转化成功的om模型文件，可以从[网站](https://modelzoo.hispark.hisilicon.com/#/ModelDetail?id=ip92pbfkp400)上进行下载。
...
4. 使用ATC工具将ONNX模型转OM模型。
    ...
    1. Hi3403V100 SVP_NNN上的om模型转换命令
        ```
        atc --framework=5 --model="./model/crnn.onnx" --input_shape="input:1,1,32,160" --out_nodes="Reshape_83:0" --output="./model/crnn" --image_list="./data/preprocess/bin/23498078_3447636521.bin;./data/preprocess/bin/31093296_629571590.bin;./data/preprocess/bin/20890750_160601314.bin;./data/preprocess/bin/30811296_2216675829.bin;./data/preprocess/bin/21029625_1100953816.bin;./data/preprocess/bin/21259312_2793608221.bin;./data/preprocess/bin/29791875_2415756793.bin;./data/preprocess/bin/31189562_2750289280.bin;./data/preprocess/bin/29099562_353962762.bin;./data/preprocess/bin/29550703_1059658537.bin;./data/preprocess/bin/24530875_1147736728.bin;./data/preprocess/bin/21303734_2984392279.bin;./data/preprocess/bin/29696125_3807376125.bin;./data/preprocess/bin/26862703_1555783028.bin" --soc_version=SS928V100 --compile_mode=6 --quant_mode=1        
        ```
    2. Hi3403V100 NNN上的om模型转换命令
        ```
        atc --framework=5 --model="./model/crnn.onnx" --input_shape="input:1,1,32,160" --output="./model/crnn" --out_nodes="Reshape_83:0" --enable_small_channel=1 --enable_single_stream=true --soc_version=OPTG
        ```
...
1.  登录到板端运行环境，切换到可执行文件main所在的目录，运行可执行文件。
    ...
    ```
    ./main --acl ../src/acl.json --model ../model/crnn.om --input ../data/file_list.json
    ```
...
3. 验证om模型的性能，参考命令如下：
    ...
    ./main --acl ../src/acl.json --model ../model/crnn.om --input ../data/file_list_1.json
    ```
    参数说明：(此模式下，file_list_1.json只放一张图片, 其中loop设为100)
    ...
    ```
    execution time: 12.31ms, frame rate: 81.26fps
    ```
    NNN平台上性能结果如下：
    ```
...
调用ACL接口推理计算，CRNN模型的性能和精度参考下列数据。
