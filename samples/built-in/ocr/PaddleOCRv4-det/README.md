...
- 输入数据
  ...
  | 输入数据 | 数据类型 | 大小             | 数据排布格式 |
  | -------- | -------- | ---------------- | ------------ |
  | image    | RGB_FP32 | 1 x 3 x 960 x 960 | NCHW         |
...
4. 切换到可执行文件main所在的目录，运行可执行文件。测试图片上模型推理命令参考：
    ...
    ```
    ./main --acl ../src/acl.json --model ../model/det.om --input ../data/file_list_1.json
    ```
...
4. 使用ATC工具将ONNX模型转OM模型。
    ...
    2. Hi3403V100 SVP_NNN上的om模型转换命令
        ```
        atc --framework=5 --model="./model/ch_ptocr_v4_det_simplified.onnx" --input_format="NCHW" --input_shape="input.1:1,3,960,960" -output="./model/det" --enable_single_stream=true --soc_version=OPTG
        ```
...
1.  登录到板端运行环境，切换到可执行文件main所在的目录，运行可执行文件。本例中，模型执行后，基于推理结果，输出的选框会保存再./out/result/txt/目录下
    ...
    ```
    ./main --acl ../src/acl.json --model ../model/det.om --input ../data/file_list.json
    ```
...
2. 精度验证。
    ...
    ```
    {'precision': 0.8614285714285715, 'recall': 0.9138605676494904, 'hmean': 0.8868702972903036}
    ```
3. 验证om模型的性能，参考命令如下：

    ```
    执行./main --acl ../src/acl.json --model ../model/det.om --input ../data/file_list_1.json
    ```
