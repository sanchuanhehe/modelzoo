...
4. 切换到可执行文件main所在的目录，运行可执行文件。本例中，模型执行后，基于推理结果，输出输入图片的top5置信度的类别标识。测试图片上模型推理命令参考：
    ...
    ```
    ./main --model ../model/InceptionV3.om --input ../data/file_list_1.json
    ```
...
1. 登录到板端运行环境，切换到可执行文件main所在的目录，执行数据集上推理命令。
    ```
    ./main --model ../model/InceptionV3.om --input ../data/file_list.json
    ```
...
3. 验证om模型的性能，参考命令如下：
    ...
    - --model：om模型路径
    - --input: 输入图片路径文件，将file_list_1.json的loop参数设置为1000
