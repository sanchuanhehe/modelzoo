...
- 参考实现：
  ...
  ```
  https://github.com/Lornatang/InceptionV3-PyTorch
  ```
...
- 输入数据
  ...
  | 输入数据 | 数据类型 | 大小                           | 数据排布格式 |
  | -------- | -------- |------------------------------| ------------ |
  | image    | RGB_FP32 | 1 x 3 x 299 x 299(SS928V100) | NCHW         |
  | image    | RGB_FP32 | 1 x 3 x 304 x 304(OPTG)      | NCHW         |
...
1. 获取权重文件。
   ...
   前往[模型](https://github.com/Lornatang/InceptionV3-PyTorch)下载对应权重，参考下载权重如下：

   [权重](https://github.com/Lornatang/InceptionV3-PyTorch?tab=readme-ov-file#download-weights)
...
2. 导出onnx文件。
    ...
    ```
    python3 ./script/pth2onnx.py ${pth_file} ${onnx_file} ${soc_version}
    ```

    参数说明：

    - pth_file：权重文件。
    - onnx_file：生成 onnx 文件。建议保存为./model/InceptionV3.onnx。
    - soc_version：处理器型号

    比如：`python3 ./script/pth2onnx.py ./model/InceptionV3-ImageNet_1K-b65ce284.pth.tar ./model/InceptionV3.onnx OPTG`
...
4.  切换到可执行文件main所在的目录，例如“$HOME/acl\_sample/out”，运行可执行文件。
    ...
    ``` 
    ./main --acl ../src/acl.json --model ../model/InceptionV3.om --input ../data/file_list.json
    ```
...
1. 精度验证。
    ...
    SVP_NNN平台上精度结果：
    ```
    {"title": "Overall statistical evaluation", "value": [{"key": "Number of images", "value": "50000"}, {"key": "Number of classes", "value": "1000"}, {"key": "Top1 accuracy", "value": "77.41%"}, {"key": "Top2 accuracy", "value": "87.05%"}, {"key": "Top3 accuracy", "value": "90.5%"}, {"key": "Top4 accuracy", "value": "92.3%"}, {"key": "Top5 accuracy", "value": "93.47%"}]}
    ```
    NNN平台上精度结果：
    ```
    {"title": "Overall statistical evaluation", "value": [{"key": "Number of images", "value": "50000"}, {"key": "Number of classes", "value": "1000"}, {"key": "Top1 accuracy", "value": "77.3%"}, {"key": "Top2 accuracy", "value": "87.03%"}, {"key": "Top3 accuracy", "value": "90.41%"}, {"key": "Top4 accuracy", "value": "92.27%"}, {"key": "Top5 accuracy", "value": "93.43%"}]}
   ```
2. 验证batch_size的om模型的性能，参考命令如下：
    执行
    ```
    ./main --acl ../src/acl.json --model ../model/InceptionV3.om --input ../data/file_list_1.json
    ```

    参数说明：(此模式下，file_list_1.json只放一张图片)

    - --model：om模型路径。
    - --input:  图片的地址文件，将file_list_1.json的loop参数设置为1000
    - --model: 模型所在位置
...
调用ACL接口推理计算，InceptionV3模型的性能和精度参考下列数据。
