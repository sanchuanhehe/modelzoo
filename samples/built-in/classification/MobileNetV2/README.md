...
- 参考实现：
  ...
  ```
  https://github.com/pytorch/vision/blob/v0.14.0/torchvision/models/mobilenetv2.py
  ```
...
4. 切换到可执行文件main所在的目录，运行可执行文件。本例中，模型执行后，基于推理结果，输出输入图片的top5置信度的类别标识。测试图片上模型推理命令参考：
    ...
    ```
    ./main --model ../model/mobileNetV2.om --input ../data/file_list_1.json
    ```
...
1. 获取权重文件。
   ...
   从开源仓获取权重文件[mobileNetV2_b0353104.pth](https://download.pytorch.org/models/mobilenet_v2-b0353104.pth)
...
2. 导出onnx文件。
    ...
    - --pth_file：权重文件。
    - --onnx_file：生成 onnx 文件。建议保存为./model/mobilenet_v2.onnx
    比如：`python ./script/pth2onnx.py ./model/mobilenet_v2-b0353104.pth ./model/mobilenet_v2.onnx`
...
1. 登录到板端运行环境，切换到可执行文件main所在的目录，执行数据集上推理命令。
    ```
    ./main --model ../model/mobileNetV2.om --input ../data/file_list.json
    ```
2. 精度验证。
    ...
    ```
    python3 ./script/accuracy.py ./out/result/txt/ ./data/val.txt ./out/accuracy.txt
    ```

    参数说明：

    - --result_dir：推理结果所在路径，默认为./out/result/txt/
    - --gt_file：真值标签文件val_label.txt所在路径。
    - --out_file：输出结果所在的位置。
    
    SVP_NNN平台上精度结果：
    ```
    {"key": "Number of images", "value": "50000"}, {"key": "Number of classes", "value": "1000"}, {"key": "Top1 accuracy", "value": "71.1%"}, {"key": "Top2 accuracy", "value": "81.64%"}, {"key": "Top3 accuracy", "value": "86.02%"}, {"key": "Top4 accuracy", "value": "88.34%"}, {"key": "Top5 accuracy", "value": "89.89%"}
    ```
    NNN平台上精度结果：
    ```
    {"title": "Overall statistical evaluation", "value": [{"key": "Number of images", "value": "50000"}, {"key": "Number of classes", "value": "1000"}, {"key": "Top1 accuracy", "value": "71.89%"}, {"key": "Top2 accuracy", "value": "82.32%"}, {"key": "Top3 accuracy", "value": "86.45%"}, {"key": "Top4 accuracy", "value": "88.87%"}, {"key": "Top5 accuracy", "value": "90.32%"}]}
    ```

3. 验证batch_size为1的om模型的性能，参考命令如下：

    ```
    ./main --model ../model/mobileNetV2.om --input ../data/file_list_1.json
    ```

    参数说明：(此模式下，输入路径file_list_1.json为一张图片)

    - --model：om模型路径。
    - --input: 图片路径存放文件，建议loop设为1000。
    ...
    ```
    [INFO] time: 3207192, fps: 311.799231
    ```

# 模型推理性能&精度<a name="ZH-CN_TOPIC_0000001172201573"></a>

调用ACL接口推理计算，mobileNetV2模型的性能和精度参考下列数据。
