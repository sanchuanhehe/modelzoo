...
在该样例中：执行sh脚本可以得到对应图片的输出结果，输出结果为图片的置信度的类别标识。
...
2. 导出onnx文件。
    ...
    - --pth_file：权重文件。
    - --onnx_file：生成 onnx 文件。建议保存为./model/densenet121.onnx

    比如：
    ```
    python3 ./script/pth2onnx.py ./model/densenet121-a639ec97.pth ./model/densenet121.onnx
    ```
...
3. 使用ATC工具将ONNX模型转OM模型。
    ...
    ```
    atc --framework=5  --model="./model/densenet121.onnx" --input_shape="input_0:1,3,224,224" --insert_op_conf="./model_cfg/SS928V100_SVP_NNN/insert_op.cfg" --output="./model/densenet121" --image_list="./data/image_ref_list.txt" --compile_mode=1 --soc_version=SS928V100
    ```
...
1. 精度验证。
    ...
    SVP_NNN平台上精度结果：
      ```
      {"title": "Overall statistical evaluation", "value": [{"key": "Number of images", "value": "50000"}, {"key": "Number of classes", "value": "1000"}, {"key": "Top1 accuracy", "value": "74.45%"}, {"key": "Top2 accuracy", "value": "84.62%"}, {"key": "Top3 accuracy", "value": "88.4%"}, {"key": "Top4 accuracy", "value": "90.57%"}, {"key": "Top5 accuracy", "value": "91.91%"}]}
      ```
    NNN平台上精度结果：

     ```
     {"title": "Overall statistical evaluation", "value": [{"key": "Number of images", "value": "50000"}, {"key": "Number of classes", "value": "1000"}, {"key": "Top1 accuracy", "value": "74.44%"}, {"key": "Top2 accuracy", "value": "84.67%"}, {"key": "Top3 accuracy", "value": "88.48%"}, {"key": "Top4 accuracy", "value": "90.55%"}, {"key": "Top5 accuracy", "value": "91.96%"}]}
     ```
...
2. 验证batch_size的om模型的性能，参考命令如下：
    ...
    ```
    [INFO] time: 586060, fps: 170.630993
    ```

    NNN平台上性能结果如下：
    ```
    [INFO] time: 9203735, fps: 108.651542
    ```
...
| ----------- | ---------- | -------- |  ------------------ | ------------------ |-----------------|
| SS928V100 SVP_NNN | 1          | ImageNet |  74.45%            | 91.91%            | 170.63   |
| SS928V100 NNN     | 1          | ImageNet |  74.44%            | 91.96%            | 108.01   |
