...
- 输入数据
  ...
  | 输入数据 | 数据类型 | 大小             | 数据排布格式 |
  | -------- | -------- | ---------------- | ------------ |
  | image    | RGB_FP32 | 1x 3 x 224 x 224 | NCHW         |
...
2. 数据预处理，将原始数据集转换为模型的输入数据。
    ...
    2.2. Hi3403V100 SVP_NNN上的数据预处理命令
    ```
...
2. 导出onnx文件。
    ...
    - --pth_file：权重文件。
    - --onnx_file：生成 onnx 文件。建议保存为./model/shufflenetv2.onnx

    比如：`python ./script/pth2onnx.py ./model/shufflenetv2_x1-5666bf0f80.pth ./model/shufflenetv2.onnx`
...
3. 使用ATC工具将ONNX模型转OM模型。
    ...
    1. Hi3403V100 SVP_NNN上的om模型转换命令
        ```
        atc --framework=5 --model="./model/shufflenetv2.onnx" --input_shape="input_0:1,3,224,224" --insert_op_conf="./model_cfg/SS928V100_SVP_NNN/insert_op.cfg" --output="./model/shufflenetv2" --image_list="./data/image_ref_list.txt" --soc_version=SS928V100  
        ```
    
    2. Hi3403V100 NNN上的om模型转换命令
        ```
        atc --framework=5 --model="./model/shufflenetv2.onnx" --input_shape="input_0:1,3,224,224" --insert_op_conf="./model_cfg/SS928V100_NNN/insert_op.cfg" --output="./model/shufflenetv2"  --enable_small_channel=1 --enable_single_stream=true --soc_version=OPTG 
        ```
      
    运行成功后生成shufflenetv2.om模型文件。
...
4.  切换到可执行文件main所在的目录，例如“$HOME/acl\_sample/out”，运行可执行文件。
    ...
    ```
    ./main --acl ../src/acl.json --model ../model/shufflenetv2.om --input ../data/file_list.txt
    ```
...
1. 精度验证。
    ...
    精度结果如下：
     ...
     SVP NNN精度验证如下：
    ```
    {"title": "Overall statistical evaluation", "value": [{"key": "Number of images", "value": "50000"}, {"key": "Number of classes", "value": "1000"}, {"key": "Top1 accuracy", "value": "68.96%"}, {"key": "Top2 accuracy", "value": "79.8%"}, {"key": "Top3 accuracy", "value": "84.05%"}, {"key": "Top4 accuracy", "value": "86.51%"}, {"key": "Top5 accuracy", "value": "88.09%"}]}
    ```
    
    dlite精度验证如下：
    ```
    {"title": "Overall statistical evaluation", "value": [{"key": "Number of images", "value": "50000"}, {"key": "Number of classes", "value": "1000"}, {"key": "Top1 accuracy", "value": "69.32%"}, {"key": "Top2 accuracy", "value": "80.01%"}, {"key": "Top3 accuracy", "value": "84.34%"}, {"key": "Top4 accuracy", "value": "86.73%"}, {"key": "Top5 accuracy", "value": "88.33%"}]}
    ``` 

2. 验证batch_size的om模型的性能，参考命令如下：

   ```
   ./main --acl ../src/acl.json --model ../model/shufflenetv2.om --input ../data/file_list_1.txt --loop 100
   ```
   ...
   dpico 性能结果如下：
   ```
   [INFO]  time: 249459, fps: 400.867

   ```

   dlite 性能结果如下：
   ```
   [INFO] time: 39217125, fps: 254.990652
   ```
...
| ----------- | ---------- | --------| ------------------ | ------------------ |---------- |
| Hi3403V100 SVP_NNN | 1  | ImageNet  | 68.96%            | 88.09%             |   400.867  |
| Hi3403V100 NNN     | 1  | ImageNet  | 69.32%            | 88.33%             |    255    |
