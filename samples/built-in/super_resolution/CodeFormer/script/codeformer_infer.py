...
def load_onnx_model(model_path):
    ...
    print(f"模型输入名称: {[x.name for x in input_info]}")
    print(f"模型输入形状: {[x.shape for x in input_info]}")
...
def run_inference(args):
    ...
    session, input_names, output_name = load_onnx_model(args.onnx_model_path)
...
def main():
    ...
    parser.add_argument('--file_list_path', default='../data/preprocess/file_list.txt',
                        help='预处理生成的文件列表路径')
    parser.add_argument('--onnx_model_path', default='../model/codeformer_simplified.onnx',
                        help='ONNX模型文件路径')
