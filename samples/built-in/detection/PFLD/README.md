...
PFLD全称A Practical Facial Landmark Detector是一个精度高，速度快，模型小的人脸关键点检测模型。
- 参考实现：
...
- 输入数据
  ...
  | 输入数据 | 数据类型 | 大小             | 数据排布格式 |
  | -------- | -------- | ---------------- | ------------ |
  | image    | BGR_FP32 | 1 x 3 x 112 x 112 | NCHW         |

- 输出数据

  | 输出数据 | 数据类型 | 大小   |
  | -------- | -------- | ------ |
  | output_1 | FP32     | 1x64x28x28 |
  | 409      | FP32     | 1x196      |
...
1. 获取原始数据集。（解压命令参考tar –xvf *.tar与 unzip *.zip）
   ...
   本模型使用[WFLW](https://wywu.github.io/projects/LAB/WFLW.html)验证集进行推理测试 ，用户自行获取数据集后，将文件解压并上传数据集modelzoo/datesets/WFLD路径下。数据集目录结构如下所示：
...
2. 数据预处理，将原始数据集转换为模型的输入数据。
    ...
    执行 源码中SetPreparation.py 脚本，完成数据预处理。
...
3. 处理图片和生成量化校准数据
    ...
    ```
    复制transformPic_quant.py和prepross.py文件到PFLD-pytorch/目录下
    生成量化校准数据
    python transformPic_quant.py 
    预处理图片文件
    python prepross.py
    ```
...
2. 导出onnx文件。
    ...
    1. 使用源码中pytorch2onnx.py导出onnx文件。
...
4.  切换到可执行文件main所在的目录，例如“$HOME/acl\_sample/out”，运行可执行文件。
    ...
    ```
    ./main --acl ../src/acl.json --model ../model/pfld.om --input ../data/file_list.txt
    ```
...
1. 精度验证。
   ...
   将posy.py文件复制到PFLD-pytorch/目录下。调用脚本可以获得精度数据。
    ...
    ```
     nme: 0.0719
     auc @ 0.1 failureThreshold: 0.3829
     failure_rate: 0.16559999999999997
    ```
2. 验证batch_size的om模型的性能，参考命令如下：

    ```
    执行./main --acl ../src/acl.json --model ../model/pfld.om --input ../data/file_list_1.txt --loop 100
    ```
    ...
    ```
     [INFO]  time: 61279, fps: 1631.880416
    ```
...
| 芯片型号    | Batch Size | 数据集   | 精度指标1（nme） | 精度指标2（auc @ 0.1 failureThreshold） | 精度指标3（failure_rate） | 性能（fps） |
| ----------- | ---------- | -------- | ------------------ | ------------------ | ------------------ | ------------------ |
| SS928V100 SVP_NNN | 1          | WFLW | 0.0719       | 0.3829       | 0.1656       | 1631.880  |