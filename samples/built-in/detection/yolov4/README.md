...
- 输出数据
  ...
  | 输出数据 | 数据类型 | 大小        | 数据排布格式        |
  | -------- | -------- | ----------- | ----------- |
  | feature_map_1  | FP32     | 1x255x76x76 | NCHW         |
  | feature_map_2  | FP32     | 1x255x38x38 | NCHW         |
  | feature_map_3  | FP32     | 1x255x19x19 | NCHW         |
...
├── script
│   ├── yolo4_preprocess.py     //数据预处理脚本
│   ├── yolo4_infer.py          //模型推理脚本
│   ├── yolo4_postprocess.py    //数据后处理脚本
│   ├── yolo4_evaluate.py       //精度评估脚本
...
1. 获取权重文件。
   ...
   在 https://github.com/Tianxiaomo/pytorch-YOLOv4 中找到yolov4.pth下载。
      ```bash
      # 创建model目录，并将下载的yolov4.pth文件放到model目录下
      mkdir model
      ```

2. 导出onnx文件。

      使用./pytorch-YOLOv4/demo_pytorch2onnx.py导出onnx模型

      ```bash
      cd pytorch-YOLOv4
   
      # yolov4.pth：权重文件。data/dog.jpg：样例图片。1 80 608 608：输入输出信息。
      python demo_pytorch2onnx.py ../model/yolov4.pth data/dog.jpg 1 80 608 608
      # 使用onnxsim简化模型
      python -m onnxsim yolov4_1_3_608_608_static.onnx yolov4_1_3_608_608_static.onnx
      # 将生成的onnx模型移动到model目录中
      cd ..
      mv pytorch-YOLOv4/yolov4_1_3_608_608_static.onnx ./model/yolov4.onnx
      ```
...
2. 验证batch_size的om模型的性能，参考命令如下：
   ...
   ```bash
   cd out # 切换到out目录
   ./main --acl ../src/acl.json --model ../model/yolov4.om  --input ../data/file_list_1.json
   ```
...
3. 使用python脚本进行模型推理结果的后处理（解码bin文件、置信度过滤、分类NMS等操作）
   ```bash
   cd script # 切换到script目录
